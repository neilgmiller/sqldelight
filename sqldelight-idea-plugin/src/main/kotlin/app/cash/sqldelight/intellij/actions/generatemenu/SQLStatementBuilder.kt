...
class SQLStatementBuilder(private val createTableStatements: List<SqlCreateTableStmt>) {
  ...
  class Result(val labelRange: TextRange, val statement: String)

  fun buildSelect(options: SelectQueryOptions): Result {
    ...
    return Result(TextRange(0, queryTitle.length), """$queryTitle:
        |SELECT $columnNames
        |FROM ${options.tableName}$whereClause;""".trimMargin())
  }

  fun buildInsert(options: InsertStatementOptions): Result {
    val queryTitle = createLabel("insert", options.table.name())

    return Result(TextRange(0, queryTitle.length), if (options.selectedColumns.size == options.table.columnDefList.size) {
  ...
  fun buildUpdate(options: UpdateStatementOptions): Result {
    ...
    return Result(
      TextRange(0, queryTitle.length), """$queryTitle:
        |UPDATE ${options.tableName}
        |SET $columnNames$whereClause;""".trimMargin())
  }

  fun buildDelete(options: DeleteStatementOptions): Result {
